;; ====================================================================
;; SISTEMA EXPERTO V3 PARA IDENTIFICACIÓN DE ANIMALES
;; ====================================================================
...
;; Función para calcular el porcentaje de coincidencia ponderado (mejorado con incompatibilidades)
      ...
      (let ((nombre (first caracteristica))
        ...
        (dolist (hecho hechos)
          (when (equal nombre (car hecho))
            ...
            (if (equal valor (cadr hecho))
                (incf peso-coincidencias peso)
                ;; Si no coincide, es incompatibilidad local
                (setf tiene-incompatibilidades t))))))
    
    ;; Si hay incompatibilidades locales (para este animal), retornar 0
    (when tiene-incompatibilidades
      (return-from calcular-porcentaje-ponderado 0.0))
...
(defun mostrar-candidatos-con-porcentajes ()
  "Muestra candidatos con porcentajes y símbolos de confianza"
  (let ((candidatos (identificar-animales-con-porcentaje-ponderado)))
    (if candidatos
        (progn
...
(defun sistema-experto ()
  "Sistema experto principal con ponderación V3"
  (format t "~%============================================~%")
  (format t "  SISTEMA EXPERTO V3 - IDENTIFICACIÓN~%")
...
(defun demo-automatica ()
  "Demostración automática con ponderación V3"
  (format t "~%=== DEMOSTRACIÓN AUTOMÁTICA V3 CON PONDERACIÓN ===~%~%")
  
  ;; Ejemplo 1: Caballo (característica definitoria: "relincha")
  (format t "Ejemplo 1: Caballo (identificación rápida por 'relincha')~%")
  (setf *hechos* '((hace-sonido relincha)))
  (let ((candidatos (identificar-animales-con-porcentaje-ponderado)))
    (format t "Hechos: hace-sonido=relincha~%")
    (format t "Candidatos encontrados: ~d~%" (length candidatos))
    (dolist (c (subseq candidatos 0 (min 3 (length candidatos))))
      (format t "  - ~a: ~,2f%%~%" (string-upcase (car c)) (cadr c)))
    (format t "~%"))
  
  ;; Ejemplo 2: Pez (características definitivas)
  (format t "Ejemplo 2: Pez (múltiples características definitivas)~%")
  (setf *hechos* '((vive-en-agua si) (tiene-escamas si) (tiene-aletas si)))
  (let ((candidatos (identificar-animales-con-porcentaje-ponderado)))
    (format t "Hechos: vive-en-agua=si, tiene-escamas=si, tiene-aletas=si~%")
    (format t "Candidatos encontrados: ~d~%" (length candidatos))
    (dolist (c candidatos)
      (format t "  - ~a: ~,2f%% ~a~%" (string-upcase (car c)) (cadr c) (simbolo-confianza (cadr c))))
    (format t "~%"))
  
  ;; Ejemplo 3: Rata (tamaño pequeño definitorio)
  (format t "Ejemplo 3: Ratón (característica 'tamaño pequeño' definitoria)~%")
  (setf *hechos* '((tamaño pequeño) (tiene-pelo si) (tiene-cola si)))
  (let ((candidatos (identificar-animales-con-porcentaje-ponderado)))
    (format t "Hechos: tamaño=pequeño, tiene-pelo=si, tiene-cola=si~%")
    (format t "Candidatos encontrados: ~d~%" (length candidatos))
    (dolist (c (subseq candidatos 0 (min 3 (length candidatos))))
      (format t "  - ~a: ~,2f%% ~a~%" (string-upcase (car c)) (cadr c) (simbolo-confianza (cadr c))))
    (format t "~%"))
  
  ;; Ejemplo 4: Incompatibilidad (relincha + agua)
  (format t "Ejemplo 4: Detección de incompatibilidad~%")
  ...
  ;; Ejemplo 5: Pájaro (múltiples características)
  (format t "Ejemplo 5: Pájaro (identificación por múltiples características)~%")
  (setf *hechos* '((tiene-plumas si) (puede-volar si) (tiene-alas si) (numero-patas 2)))
  (let ((candidatos (identificar-animales-con-porcentaje-ponderado)))
    (format t "Hechos: tiene-plumas=si, puede-volar=si, tiene-alas=si, numero-patas=2~%")
    (format t "Candidatos encontrados: ~d~%" (length candidatos))
    (dolist (c candidatos)
      (format t "  - ~a: ~,2f%% ~a~%" (string-upcase (car c)) (cadr c) (simbolo-confianza (cadr c))))
    (format t "~%"))
  
  ;; Ejemplo 6: León (características de depredador)
  (format t "Ejemplo 6: León (depredador con rugido)~%")
  (setf *hechos* '((hace-sonido rugido) (es-carnivoro si) (tamaño grande) (habilidad-caza si)))
  (let ((candidatos (identificar-animales-con-porcentaje-ponderado)))
    (format t "Hechos: hace-sonido=rugido, es-carnivoro=si, tamaño=grande, habilidad-caza=si~%")
    (format t "Candidatos encontrados: ~d~%" (length candidatos))
    (dolist (c candidatos)
      (format t "  - ~a: ~,2f%% ~a~%" (string-upcase (car c)) (cadr c) (simbolo-confianza (cadr c))))
    (format t "~%")))
